	...
	<div class="custom-learndash-list custom-learndash-my-courses-list">
		<div class="ld-dashboard-course-content instructor-courses-list"> 
			<div class="ld-dashboard-section-head-title">
				<h3 class="ld-dashboard-nav-title"><?php printf( esc_html__( 'My %s', 'ld-dashboard' ), esc_html( LearnDash_Custom_Label::get_label( 'questions' ) ) ); ?></h3>
				<div class="ld-dashboard-header-button ld-dashboard-add-new-button-container">
					<?php $question_nonce = wp_create_nonce( 'question-nonce' ); ?>
					<a class="ld-dashboard-add-course ld-dashboard-add-new-button ld-dashboard-btn-bg" href="<?php echo esc_url( $dashboard_page_url ); ?>?action=add-question&tab=my-questions&_lddnonce=<?php echo esc_attr( $question_nonce ); ?>">
					<span class="material-symbols-outlined add-icon"><?php esc_html_e( 'add', 'ld-dashboard' ); ?></span> <?php printf( esc_html__( 'Add A New %s', 'ld-dashboard' ), esc_html( LearnDash_Custom_Label::get_label( 'question' ) ) ); ?></a>
				</div>
			</div>
			<div class="ld-dashboard-content-inner">
				<?php do_action( 'ld_dashboard_before_quiz_filter' ); ?>
				<div class="ld-dasboard-my-question-filter ld-dashboard-course-filter">
					...
					<div class="ld-dashboard-actions-iteam">
						<label><?php printf( esc_html__( 'All %s', 'ld-dashboard' ), esc_html( LearnDash_Custom_Label::get_label( 'quiz' ) ) ); ?></label>
						<select class="ld-dashboard-sec-filter ld-dashboard-quiz-filter-select"></select>
					</div>
					<button class="ld-dashboard-course-filter-submit ld-dashboard-btn-bg" data-type="question"><?php esc_html_e( 'Filter', 'ld-dashboard' ); ?></button>
				</div>
				<?php do_action( 'ld_dashboard_before_questions_content' ); ?>
				<div class="ld-dashboard-tab-content-wrapper"></div>
				<?php do_action( 'ld_dashboard_after_questions_content' ); ?>
			</div>
		</div>
